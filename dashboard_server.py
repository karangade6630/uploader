...
A lightweight Flask SSE (Server-Sent Events) server that:
  ...
  - Stores and exposes MongoDB status via GET /api/db-status
  - Auto-opens the browser on local start
"""
...
# ---------------------------------------------------------------------------
# Flask import with helpful error message
# ---------------------------------------------------------------------------
try:
...
# Optional callbacks registered by colab_demo.py
...
# ---------------------------------------------------------------------------
# MongoDB state - updated by colab_demo.py via update_db_state()
# ---------------------------------------------------------------------------
_db_state: dict = {
    "connected": False,
    "total_movies": 0,
...
def update_db_state(data: dict):
    """Called by colab_demo.py after a MongoDB overview scan to update cached state."""
    with _db_state_lock:
        _db_state.update(data)
        _db_state["last_checked"] = datetime.now().isoformat()
...
def _set_state(new_state: str):
    ...
    emit("ctrl_state", state=new_state, msg=f"Control changed to {new_state}")
...
def _broadcast(event: dict):
    """Push an event to every connected SSE client and to history."""
    with _history_lock:
...
def receive_log():
    ...
    try:
        ...
        _broadcast(data)
        # If this is a db_status event, update our cached state too
        if data.get("type") == "db_status":
            update_db_state(data)
    ...
    if action == "start":
        if state in ("idle", "stopping"):
            ...
            if _on_start_callback:
                t = threading.Thread(
                    target=_on_start_callback, daemon=True, name="runner"
                )
                t.start()
        ...
        if state == "running":
            _pause_event.clear()    # colab_demo.py blocks on _pause_event.wait()
            _set_state("paused")
...
@app.route("/control/check-db", methods=["POST"])
def trigger_db_check():
    """
    Dashboard can POST here to trigger an on-demand MongoDB overview scan.
    Runs in a background thread so the HTTP response returns immediately.
    """
    try:
        import colab_demo
        threading.Thread(target=colab_demo.get_mongo_overview, daemon=True).start()
        return jsonify({"ok": True, "msg": "DB check started in background"})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
def db_status():
    """Returns the latest cached MongoDB overview state."""
    with _db_state_lock:
        return jsonify(dict(_db_state))
...
def sse_stream():
    ...
    def generate():
        ...
        # Send current DB state immediately
        with _db_state_lock:
            db_snap = dict(_db_state)
        db_snap["type"] = "db_status"
        db_snap.setdefault("ts", datetime.now().isoformat())
        yield _make_sse(db_snap)
...
def start_server(open_browser: bool = True):
    ...
    if open_browser and os.environ.get("RENDER") != "true":
